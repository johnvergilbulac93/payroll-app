...
import { index as loanTypeIndex, store as loanTypeStore } from '@/routes/masterfile/loanType';
import { index as employeeIndex } from '@/routes/masterfile/employees';
...
onMounted(() => {
    ...
    if (props.loans) {
        form.id = props.loans.id;
        form.EmpNbr = props.loans.EmpNbr;
        form.LoanTypeID = props.loans.LoanTypeID;
        form.OrigBal = props.loans.OrigBal ? props.loans.OrigBal : 0;
        form.DedAmt = props.loans.DedAmt ? props.loans.DedAmt : 0;
        form.StartDate = stringToDateValue(props.loans.StartDate);
        form.Frequency = props.loans.Frequency;
        form.BalanceAmt = props.loans.BalanceAmt ? props.loans.BalanceAmt : 0;
...
const onSaveLoanType = () => {
    loanType.post(loanTypeStore(), {
        onSuccess: () => {
            loanType.reset('LoanType');
            visible.value = false;
            toast.success('Loan Type successfully added. ');
        },
    });
};
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="px-4 py-6">
            <div class="flex items-center justify-between">
                <Heading :title="isAdd ? ' New Employee Loan' : 'Update Employee Loan'" :description="isAdd
            ...
            <div class="space-y-4">
                <div class="grid gap-4 sm:grid-cols-1 lg:grid-cols-4 ">
                    <div class="grid grid-rows-[auto_auto_20px] gap-2 ">
                        ...
                        <div class="flex items-center gap-2 w-[210px] ">
                            <ComboboxServerSideSearch :loading="isLoadingSearch" @search="onSearchLoanType"
                                :items="loanTypes" v-model="form.LoanTypeID" :labelText="'Select a Loan Type'" />
                            <Tooltip description="Add Loan Type">
                                <Button @click="showLoanTypeDialog" variant="outline" class="cursor-pointer">
                    ...
                    <div class="grid grid-rows-[auto_auto_20px] gap-2 col-span-3 ">
                        ...
                        <div class="w-64">
                            <ComboboxServerSideSearch :loading="isLoadingSearch" @search="onSearchEmployee"
                                :items="employees" v-model="form.EmpNbr" :labelText="'Select a employee'" />
                        </div>
                ...
                <div class="grid gap-4 sm:grid-cols-1 lg:grid-cols-3">
                    <div class="grid grid-rows-[auto_auto_15px] gap-2">
                        ...
                        <NumberField v-model="form.OrigBal" class="mt-1 block w-full" id="original_balance" :step="0.01"
                    ...
                    <div class="grid grid-rows-[auto_auto_15px] gap-2">
                        ...
                        <NumberField v-model="form.DedAmt" class="mt-1 block w-full" id="deduction_amount" :step="0.01"
                            :locale="'en-PH'" :format-options="{
                                style: 'currency',
                                currency: 'PHP',
                                minimumFractionDigits: 2,
                    ...
                    <div class="grid grid-rows-[auto_auto_15px] gap-2">
                        ...
                        <NumberField v-model="form.BalanceAmt" class="mt-1 block w-full" id="balance_amount"
                            :step="0.01" :locale="'en-PH'" :format-options="{
                                style: 'currency',
                                currency: 'PHP',
                                minimumFractionDigits: 2,
                                maximumFractionDigits: 2,
                ...
                <div class="grid gap-4 sm:grid-cols-1 lg:grid-cols-3">
                    <div class="grid grid-rows-[auto_auto_20px] gap-2 ">
                        ...
                        <Input id="frequency" v-model="form.Frequency" type="text" class="mt-1 block w-full"
                            autocomplete="frequency" placeholder="Frequency" />
                        <InputError :message="props.errors.Frequency" />
                    </div>

                    <div class="grid grid-rows-[auto_auto_20px] gap-2">
                        <Label for="start_deduction_date">Start Deduction Date</Label>
                        <DatePicker class="w-50" v-model="form.StartDate"></DatePicker>
                        <InputError :message="props.errors.StartDate" />

                    </div>
                    <div class="grid grid-rows-[auto_auto_20px] gap-2">
                        <Label for="balance_as_of_date">Balance As of Date</Label>
                        <NumberField v-model="form.BalanceasofDate" class="mt-1 block w-full" id="balance_as_of_date"
                            :step="0.01" :locale="'en-PH'" :format-options="{
                                style: 'currency',
                                currency: 'PHP',
                                minimumFractionDigits: 2,
        ...
        <Dialog v-model:visible="visible" title="New Loan Type" description="Manage a new loan type"
            @save="onSaveLoanType">
            <div class="grid gap-4">
                <div class="grid gap-2">
                    <Label for="loan_type">Loan Type</Label>
                    <Input v-model="loanType.LoanType" id="loan_type" name="loan_type" type="text"
